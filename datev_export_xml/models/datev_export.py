    ...
    def _check_partner_data(self, invoice):
        ...
        def check_field(field_value, field_name, partner_name):
            if field_value and re.search(invalid_chars_pattern, field_value):
                ...
                char_codes = ["U+{:04X}".format(ord(char)) for char in invalid_chars]
                raise ValueError(
                    _(
        ...
        if invoice.move_type in ["out_invoice", "out_refund"]:
            # For outgoing invoices: check customer (invoice_party) and company (supplier_party)
            partners_to_check.append((invoice.partner_id, "Customer"))
            partners_to_check.append((invoice.company_id.partner_id, "Company"))
        else:
            # For incoming invoices: check vendor (supplier_party) and company (invoice_party)
            partners_to_check.append((invoice.partner_id, "Vendor"))
            partners_to_check.append((invoice.company_id.partner_id, "Company"))
